"""Pure-Python tests for `current_veg_index_years()` (app/domain/
analysis_config.py, Wave: enriched vegetation-index results, bug fix).
...
`current_veg_index_years(today)` fixes both: it is a function (evaluated
...
def test_lower_bound_is_always_the_2017_sentinel_2_floor():
    for today in (date(2018, 1, 1), date(2026, 5, 31), date(2030, 12, 31)):
        assert min(current_veg_index_years(today)) == _VEG_INDEX_FIRST_YEAR


def test_returns_a_contiguous_range_not_a_list_with_gaps():
    years = current_veg_index_years(date(2026, 8, 8))
